...
AGGREGATE_OUT_DIR = "./Task602_metrics_all_models"

# 七个模型在 Task602 上的预测目录，根据你实际保存的位置稍作调整
MODELS = {
    # 模型名: preds 目录
    ...
    # # "UNet3D": "/home/fangzheng/zoule/ESO_nnUNet_dataset/nnUNet_predictions/Task602_ls/UNet3DTrainer/preds",
}
...
def compute_tp_fp_fn_per_case(pred: np.ndarray, gt: np.ndarray, label_ids=None):
    ...
    if label_ids is None:
        # For Task602 we only care about foreground label 1; ignore background 0
...
def evaluate_one_model(model_name: str, pred_dir: str):
    ...
    for cid in case_ids:
        ...
        tp_fp_fn = compute_tp_fp_fn_per_case(pred, gt, label_ids=[1])  # Task602 foreground label=1
    ...
    # summary.json
    summary = OrderedDict()
    summary["author"] = "Task602_eval"
    summary["description"] = (
        f"Evaluation summary for Task602_ls using predictions from {pred_dir} (model={model_name})"
    )
    summary["id"] = datetime.now().strftime("%Y%m%d%H%M%S")
    summary["name"] = "Task602_ls"
    summary["results"] = {
        "all": per_case_results,
        "mean": mean_results,
    }
    summary["task"] = "Task602_ls"
    summary["timestamp"] = str(datetime.now())

    os.makedirs(pred_dir, exist_ok=True)
    summary_path = os.path.join(pred_dir, f"summary_task602_{model_name}.json")
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=4)
    print(f"[MODEL] {model_name}: saved summary to {summary_path}")

    # CSV: per-case metrics
    cases_csv = os.path.join(pred_dir, f"metrics_task602_{model_name}_cases.csv")
    ...
    mean_csv = os.path.join(pred_dir, f"metrics_task602_{model_name}_mean.csv")
    ...
    tp_fp_fn_csv = os.path.join(pred_dir, f"metrics_task602_{model_name}_cases_tp_fp_fn.csv")
...
def main():
    ...
    if all_rows_cases and global_metric_names is not None:
        ...
        agg_cases_csv = os.path.join(AGGREGATE_OUT_DIR, "metrics_task602_all_models_cases.csv")
    ...
    if all_rows_mean and global_metric_names is not None:
        ...
        agg_mean_csv = os.path.join(AGGREGATE_OUT_DIR, "metrics_task602_all_models_mean.csv")
    ...
    if all_rows_cases_tp_fp_fn:
        ...
        agg_tp_csv = os.path.join(AGGREGATE_OUT_DIR, "metrics_task602_all_models_cases_tp_fp_fn.csv")
        tp_fieldnames = ["model", "case_id", "label", "TP", "FP", "FN", "n_pred", "n_gt"]
